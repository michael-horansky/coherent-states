...
class BH():
    ...
    def iterate(self, max_t, dt):
        ...
        for i in range(self.N):
            A[i] = (identity_prefactor / np.power(1.0 + np.sum(np.conjugate(self.basis[i].z) * self.basis[i].z), self.M + self.S)) * np.power(self.beta * self.beta / np.pi, self.M - 1) * self.z_0.overlap(self.basis[i])
        return(A)

z_0 = 0.0+ 1j * 0.0

lol = BH(1, 0.5, 2 * np.pi, 0.1, 0, 0, 5, 2)
lol.sample_gridlike(15, np.array([z_0], dtype=complex), 0.1)
