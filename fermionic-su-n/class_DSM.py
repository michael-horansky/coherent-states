...
class DSM():
    ...
    def save_datum(self, data_group, datum_name):
        ...
        if self.is_data_initialised[data_group][datum_name]:
            ...
            if not (self.metadata[data_group][datum_name] is None or self.metadata[data_group][datum_name] == {}):
                metadatum_file = open(self.metadata_directory(data_group, datum_name), DSM.object_type_write_mode["pkl"])
    ...
    def load_datum(self, data_group, datum_name):
        ...
        if metadatum_path.is_file():
            metadatum_file = open(self.metadata_directory(data_group, datum_name), DSM.object_type_read_mode["pkl"])
    ...
    def save_root_metadata(self):
        ...
        metadatum_file = open(f"{self.storage_path}/root_meta.json", DSM.object_type_write_mode["json"])
    ...
    def load_root_metadata(self):
        metadatum_file = open(f"{self.storage_path}/root_meta.json", DSM.object_type_read_mode["json"])
