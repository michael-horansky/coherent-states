...
# Standard sep
mol_solvers[1.0] = ground_state_solver(f"{cur_molecule}_{ds_id}_dist=1.0")
...
for i in range(len(nontrivial_separations)):
    ...
    mol_solvers[separation_coef] = ground_state_solver(f"{cur_molecule}_{ds_id}_dist={separation_coef}")
...
for i_ds in range(len(datasets_to_load)):
    ...
    ax1.plot(E_cols["c"], E_cols[ds]["E_g"], label = ds, color = cmap(i_ds + 3))
